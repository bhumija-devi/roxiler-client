...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [statsResponse, barChartResponse, tableResponse] =
          await Promise.all([
            axios.get(
              `https://roxiler-server.vercel.app/statistics?month=${selectedMonth}`
            ),
            axios.get(
              `https://roxiler-server.vercel.app/bar-chart?month=${selectedMonth}`
            ),
            axios.get(
              `https://roxiler-server.vercel.app/tabledata?month=${selectedMonth}`
            ),
          ]);
  ...
  return (
    <div className="App">
      ...
      <div className="select-month-con">
        ...
        />
        <div>
          <label htmlFor="monthDropdown" className="select-month-head">
            Select Month:{" "}
          </label>
          <select
            id="monthDropdown"
